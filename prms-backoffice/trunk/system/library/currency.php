...
final class Currency {
  	...
  	public function format($number, $currency = '', $value = '', $format = TRUE) {
    	...
    	if ($value) {
      		...
      		$value = 1;    // TEMP HACK TO PREVENT CURRENCY CONVERSION -gc
    	}
      //echo '<!--'.$number.':'.$currency.':'.$value.':'.$format.'-->';
    	if ($value) {
      	$value = $number * $value;
    	} else {
      	$value = $number;
      ...
      //KMC hack, the above code was chopping off values like 1,695.00, this simplifices things.
      //Problem is that PHP doesn't convert number strings with , commas in them.
