			...
			<?php if (@$calendar_events) { ?>
				...
				<?php foreach ($calendar_events as $calendar_event) { ?>
					...
					<tr class="<?php echo $class; ?>">
						<td align="center">
							<?php if ($calendar_event['delete']) { ?>
								<input type="checkbox" name="delete[]" value="<?php echo $calendar_event['calendar_id']; ?>" checked="checked" />
							<?php } else { ?>
								<input type="checkbox" name="delete[]" value="<?php echo $calendar_event['calendar_id']; ?>" />
							<?php } ?>
						</td>
						<td class="left"><?php echo $calendar_event['title']; ?></td>
						<td class="right"><?php echo $calendar_event['status']; ?></td>
						<td class="right">
							<?php foreach ($calendar_event['action'] as $action) { ?>
