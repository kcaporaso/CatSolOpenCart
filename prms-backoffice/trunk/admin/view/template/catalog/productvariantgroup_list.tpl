...
<form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
  <table class="list">
    ...
    <tbody>
      <?php if ($productvariantgroups) { ?>
          <?php $class = 'odd'; ?>
          <?php foreach ($productvariantgroups as $productvariantgroup) { ?>
              <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
              <tr class="<?php echo $class; ?>">
              
                <td align="center">
                  <?php if ($productvariantgroup['delete']) { ?>
                    <input type="checkbox" name="delete[]" value="<?php echo $productvariantgroup['id']; ?>" checked="checked" />
                  <?php } else { ?>
                    <input type="checkbox" name="delete[]" value="<?php echo $productvariantgroup['id']; ?>" />
                  <?php } ?>
                </td>
                
                <td class="right">
                	<?php echo $productvariantgroup['id']; ?>
                </td>                
                
                <td class="left">
                	<?php echo $productvariantgroup['name']; ?>
                </td>
                
                <td class="right"><?php foreach ($productvariantgroup['action'] as $action) { ?>
