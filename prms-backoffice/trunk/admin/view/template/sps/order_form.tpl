...
<div id="order">
  ...
  <div id="order_details">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
          <?php echo $payment_method; ?><br /> <?php if (!empty($update_payment_url)) {?> <a id="update_payment_url" onclick="$('#update_payment').show('slow'); $('#update_payment_url').hide();">[Update]</a><?php } ?>
          <?php if (!empty($po_school_name))    { echo $po_school_name . '<br/>'; } ?>
          <?php if (!empty($po_account_number)) { echo 'Acct#' . $po_account_number . '<br/>'; } ?>
          <?php if (!empty($po_number)) { echo 'PO#' . $po_number . '<br/>'; } ?>
          </td>
    ...
    <div id="update_payment">
       <br/>
       <table id="payment_methods">
       <thead>
          <tr><td id="method_td">Pick a method and enter the required information.<span style="position:relative;float:right;"><a style="color:white;" onclick="$('#update_payment').hide('slow'); $('#update_payment_url').show('slow');">Close</a></span></td></tr>
       </thead>
       <tbody>
          <tr><td><a onclick="$('#purchase_order_information').hide(); $('#credit_card_information').show('slow');"><img src="<?php echo HTTP_SERVER ?>/view/image/add.png" /></a> Credit Card Information</td></tr>
              ...
              <table>
                 ...
                 <tbody>
                  <tr>
                    ...
                    <td><input type="text" name="cc_expire_date_month" maxlength="2" size="3"/>/<input type="text" name="cc_expire_date_year" maxlength="4" size="5"/></td>
                    <td>
  	                    <a onclick="alert('Save Credit Card Information');" class="button"><span class="button_left button_save"></span><span class="button_middle">Save Credit Card Information</span><span class="button_right"></span></a>
                    </td>
                  </tr>
                 </tbody>
              </table> 
          ...
          <tr><td><a onclick="$('#purchase_order_information').show('slow'); $('#credit_card_information').hide();"><img src="<?php echo HTTP_SERVER ?>/view/image/add.png" /></a> Purchase Order Information</td></tr>
          <tr id="purchase_order_information">
           <td>
              ...
              <tbody>
              <tr>
                 <td><input type="input" id="purchase_order_number" name="purchase_order_number" value=""/></td>
                 <td><input type="input" id="purchase_order_account_number" name="purchase_order_account_number" value=""/></td>
                 <td>
  	                 <a onclick="save_po_information();" class="button"><span class="button_left button_save"></span><span class="button_middle">Save PO Information</span><span class="button_right"></span></a>
                 </td>
          ...
          <td>
            <?php if (strstr($total['title'], 'Shipping')) { ?>
               <?php if (strstr($total['title'], 'Shipping Adjustment')) { 
                  $need_adjust = 0;
               ?>
                  <a id="remove_adjust_shipping" onclick="remove_adjust_shipping(<?php echo $subtotals_index; ?>);" title="Remove Shipping Adjustment"><img src="<?php echo HTTP_SERVER; ?>/view/image/delete.png" /></a>
                  <input type="hidden" name="shipping_adjustment" value="<?php echo $total['value']; ?>"/>
                  
               <?php } 
                  if ($need_adjust) { ?>
                  <a id="add_adjust_shipping" onclick="adjust_shipping(<?php echo $subtotals_index; ?>);" title="Adjust Shipping"><img src="<?php echo HTTP_SERVER; ?>/view/image/add.png" /></a>
               <?php } ?>
            <?php } ?>
          </td>
        </tr>
    ...
    <!-- SHOW SPS APPROVERS HERE -->
    <table>
      <thead>
        <tr>
          <td align="center" colspan="3"><?php echo $text_order_approvers; ?></td>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td width="140"><b><?php echo $text_approver_name; ?></b></td>
          <td><b><?php echo $text_approver_role; ?></b></td>
          <td><b><?php echo $text_approver_action; ?></b></td>
        </tr>
        <?php foreach ($order_approvers as $approver) { ?>
        <tr>
          <td><?php echo $approver['firstname'].' '.$approver['lastname']; ?>
              <?php 
          ...
          <td>
              <?php echo $approver['role_name']; ?>
          </td>
          <td>
            <div>
             <?php 
             // check for approval audit 
             $audit_found_for_user = false;
             if (count($approval_audits)) {
                foreach ($approval_audits as $audit) {
                   if ($approver['user_id'] == $audit['user_id']) {
                      $audit_found_for_user = true;
                      echo $audit['status_name'] . ' on ';
                      echo date($this->language->get('date_format_short'), strtotime($audit['date_added']));
                      echo '<br/>';
                   }
                }
             }
             // We're going to keep the buttons there all the time so minds can be changed.
             ?>
             </div>
             <div class="buttons" id="order_action_buttons">
             <?php
             if ($approver['user_id'] == $user_id) { 
             ?>
  	          <a onclick="approve_order();" class="button"><span class="button_left button_save"></span><span class="button_middle"><?php echo $button_approve; ?></span><span class="button_right"></span></a>
           	 <a onclick="reject_order();" class="button" style="margin-left: 5px;"><span class="button_left button_cancel"></span><span class="button_middle"><?php echo $button_reject; ?></span><span class="button_right"></span></a>
             <?php } ?>
            </div>
          </td>
        </tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
            <td><select name="order_status_id" id="order_status_id">
			...
			<td style="border-left:0;">
            	<span class="help">Shoppers will be notified by email and this Order Status change will be visible to them in their Order Invoice. 
            		Shoppers will always see a different version of the Order Status, the one shown in square brackets on the left.
            	</span>
          	</td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="9"><textarea id="comment" name="comment" cols="40" rows="8" style="width: 99%"><?php echo $comment; ?></textarea></td>
        </tr>
        <tr>
    ...
    <input type="hidden" id="approval_step" name="approval_step" value=""/> 
    <input type="hidden" id="update_payment_info" name="update_payment_info" value=""/> 
    <input type="hidden" id="update_payment_type" name="update_payment_type" value=""/> 
  </form>
</div>
...
<script type="text/javascript" src="<?php echo HTTP_SERVER ?>/view/javascript/jquery/jquery.formatCurrency-1.1.0.js"></script>
...
$().ready(function() {
   ...
   $('#print-order').click(function() {
      ...
      $('<p style="text-align:center;font-weight:bold;">Bender-Burkot.com<br/>Bender-Burkot East Coast School Supply Corporation<br/>Hwy. 17 North, P.O. Box Box 147 | Pollocksville, North Carolina 28573<br/>Toll Free: 800-682-2638 | Toll Free Fax: 800-717-2277</p>').prependTo('#print-order-clone');
...
function approve_order() {
   ...
   $('#approval_step').val('SPS_APPROVE');
   $('#form').submit();
}

function reject_order() {
   // change the status to pending
   $('#approval_step').val('SPS_REJECT');
   $('#order_status_id').val(11);
   $('#notify').attr('checked', true);
   if ($('#comment').val().length == 0) {
      alert('If you Reject an order you must supply a comment.'); 
      $('#comment').focus();
      return;
   } else {
      $('#form').submit();
...
function adjust_shipping(row) {
   title = "Shipping Adjustment:";
	...
	html += '<td style="background-color:#ddd;"><a onclick="remove_adjust_shipping('+subtotal_row+');" title="Remove Shipping Adjustment"><img src="<?php echo HTTP_SERVER ?>/view/image/delete.png"/></a></td>';
