...
class ControllerSettingSetting extends Controller {
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			//KMC Not using, Andrea load an image directly to ftp site:: $this->transform_store_navbackground_image($_SESSION['store_code'], $this->request->post['config_navbackground_color']);
		...
		$this->data['entry_home_page'] = $this->language->get('entry_home_page');
		...
		$this->data['error_home_page'] = @$this->error['home_page'];
		...
		if (isset($this->request->post['config_home_page'])) {
			$this->data['config_home_page'] = $this->request->post['config_home_page'];
		} else {
			$this->data['config_home_page'] = $this->config->get('config_home_page');
		}
		...
		if (isset($this->request->post['config_nonstandard_products'])) {
			$this->data['config_nonstandard_products'] = $this->request->post['config_nonstandard_products'];
		} else {
		...
		$this->data['informations'] = $this->model_catalog_information->getInformations($_SESSION['store_code']);
		...
		/*if (isset($this->request->post['config_navbackground_color'])) {
			$this->data['config_navbackground_color'] = $this->request->post['config_navbackground_color'];
		} else {
			$this->data['config_navbackground_color'] = $this->config->get('config_navbackground_color');
		}*/		
		...
		if (isset($this->request->post['config_googleanalytics_code'])) {
			$this->data['config_googleanalytics_code'] = $this->request->post['config_googleanalytics_code'];
		} else {
			$this->data['config_googleanalytics_code'] = $this->config->get('config_googleanalytics_code');
		}		
		...
		if (isset($this->request->post['config_share_facebook'])) {
			$this->data['config_share_facebook'] = $this->request->post['config_share_facebook'];
		} else {
			$this->data['config_share_facebook'] = $this->config->get('config_share_facebook');
		}

		if (isset($this->request->post['config_share_twitter'])) {
			$this->data['config_share_twitter'] = $this->request->post['config_share_twitter'];
		} else {
			$this->data['config_share_twitter'] = $this->config->get('config_share_twitter');
		}

		if (isset($this->request->post['config_min_purchase'])) {
			$this->data['config_min_purchase'] = $this->request->post['config_min_purchase'];
		} else {
			$this->data['config_min_purchase'] = $this->config->get('config_min_purchase');
		}
	...
	private function validate() {
		...
		if (!$this->request->post['config_store']) {
      	...
      	if (strlen(utf8_decode($this->request->post['config_meta_description'])) > 132) {
    	...
    	/*if ((strlen(utf8_decode($this->request->post['config_email'])) > 250) || (!preg_match('#^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$#i', $this->request->post['config_email']))) {
	...
	public function logo() {
		...
		if (isset($this->request->get['logo'])) {
			...
			if ((!isset($this->request->server['HTTPS'])) || ($this->request->server['HTTPS'] != 'on')) {
				$image = HTTP_IMAGE .'stores/'.$_SESSION['store_code'].'/'. basename($this->request->get['logo']);
			} else {
				$image = HTTPS_IMAGE .'stores/'.$_SESSION['store_code'].'/'. basename($this->request->get['logo']);
			}					
	...
	public function transform_store_navbackground_image ($store_code, $hexcolor) {
        ...
        $image_source_path = DIR_IMAGE.'nav_background.png';
        $image_dest_path = DIR_IMAGE.'stores/'.$store_code.'/nav_background.png';
        chmod($image_dest_path, 0777);
//echo $image_dest_path;
        
        $image = imagecreatefrompng($image_source_path);
        if (!$image) { echo "WHOOPS, missing a library?"; exit;}

        if (imagefilter($image, IMG_FILTER_COLORIZE, $R, $G, $B)) {
            //imagepng($image, $image_dest_path);
        }
