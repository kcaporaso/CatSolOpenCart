...
class ControllerSPSOrder extends Controller {
	...
	private $error = array();
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate($this->request->get['order_id'], $this->user->getID()) ) {
			...
			$this->model_sps_order->editOrder($_SESSION['store_code'], $this->request->get['order_id'], $this->request->post);
			...
			$this->redirect($this->url->https('sps/order' . $url));
    	}
  	...
  	public function delete() {
    	...
    	if ( (isset($this->request->post['delete'])) && $this->validate($this->request->get['order_id'], $this->user->getID()) ) {
			foreach ($this->request->post['delete'] as $order_id) {
				$this->model_sps_order->deleteOrder($_SESSION['store_code'], $order_id);
			}	
  	...
  	private function getList() {
		...
		$date_filter = strtotime($date_range['year'].'-'.$date_range['month'].'-01 00:00:00');
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => $this->url->https('sps/order' . $url),
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
							
      // no longer delete from here... just go to the order page, default action for list form.
		$this->data['delete'] = $this->url->https('sps/order' . $url);	
		...
		$data = array(
			...
			'school_name'     => @$this->request->get['filter_school_name'], 
			'district_name'   => @$this->request->get['filter_district_name'], 
      ...
      // Filter on this user's school id if not an admin
      if (!$this->user->getSPS()->isAdmin()) {
         $data['school_id'] = $this->user->getSPS()->getSchoolID();
      }

		$order_total = $this->model_sps_order->getTotalOrders($_SESSION['store_code'], $data);
		$results = $this->model_sps_order->getOrders($_SESSION['store_code'], $data);
      ...
      $this->load->model('sps/user');
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
   			...
   			//'href' => $this->url->https('sps/order/update&order_id=' . $result['order_id'] . $url . '&ADMIN_SESSION_ID=' . session_id())
   			'href' => $this->url->https('sps/order/update&order_id=' . $result['order_id'] . $url)
			);
         $waiting_on_user = '';
		   if ($result['waiting_on']) {
            $w = $this->model_sps_user->getUser($result['waiting_on']);   
            $waiting_on_user = $w['firstname'] .' '. $w['lastname'];
         }   
			$this->data['orders'][] = array(
            ...
            'districtname' => $this->model_sps_school->getDistrictNameForSchoolId($result['school_id']),
				'action'     => $action
			);
		...
		if (isset($this->request->get['filter_school_name'])) {
			$url .= '&filter_school_name=' . $this->request->get['filter_school_name'];
		}
		...
		$this->data['sort_order'] = $this->url->https('sps/order&sort=o.order_id' . $url);
		$this->data['sort_name'] = $this->url->https('sps/order&sort=name' . $url);
		$this->data['sort_status'] = $this->url->https('sps/order&sort=status' . $url);
		$this->data['sort_date_added'] = $this->url->https('sps/order&sort=o.date_added' . $url);
		$this->data['sort_total'] = $this->url->https('sps/order&sort=o.total' . $url);
		$this->data['sort_district'] = $this->url->https('sps/order&sort=district' . $url);
		...
		$pagination->url = $this->url->https('sps/order' . $url . '&page=%s');
			
		$this->data['pagination'] = $pagination->render();
		
		$this->data['filter_order_id'] = @$this->request->get['filter_order_id'];
		$this->data['filter_name'] = @$this->request->get['filter_name'];
		$this->data['filter_school_name'] = @$this->request->get['filter_school_name'];
		$this->data['filter_district_name'] = @$this->request->get['filter_district_name'];
    	...
    	$this->data['order_statuses'] = $this->model_sps_order_status->getOrderStatuses();
				
		$this->data['sort'] = $sort;
		$this->data['order'] = $order;

		$this->id       = 'content';
		$this->template = 'sps/order_list.tpl';
  	...
  	private function getForm() {
		...
		$this->data['text_order_approvers'] = $this->language->get('text_order_approvers');
		...
		$this->data['text_approver_name'] = $this->language->get('text_approver_name');
		$this->data['text_approver_action'] = $this->language->get('text_approver_action');
		$this->data['text_approver_role'] = $this->language->get('text_approver_role');
		...
		if (isset($this->request->get['order'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => $this->url->https('sps/order'),
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
		
    	$this->data['action'] = $this->url->https('sps/order/update&order_id=' . (int)@$this->request->get['order_id'] . $url);
      if (isset($this->request->get['cancel_url'])) {
    	   $this->data['cancel'] = $this->request->get['cancel_url'];
      } else {
    	   $this->data['cancel'] = $this->url->https('sps/order' . $url);
      }
		$this->data['invoice'] = $this->url->https('sps/order/invoice&order_id=' . (int)@$this->request->get['order_id']);		
		...
		$this->data['update_subtotals_action'] = $this->url->https('sps/order/get_order_subtotals_display_data');
		
    	$order_info = $this->model_sps_order->getOrder($_SESSION['store_code'], $this->request->get['order_id']);
      ...
      if ($this->data['payment_method'] == 'Skip Payment') {
         $this->data['update_payment_url'] = $this->url->https('sps/order/update_payment');
      }
      ...
      if (isset($order_info['po_number']))
      {
         $this->data['po_number'] = $order_info['po_number'];
      }
		
      if ($order_info['payment_method'] == 'Credit Card') {
   		$this->load->library('encryption');		
   		$encryption = new Encryption($this->config->get('config_encryption'));		
   		
   		$payment_row = $this->model_sps_order->getCCCaptureRow($_SESSION['store_code'], $this->request->get['order_id']);
    	...
    	foreach ($products as $product) {
			...
			//$options = $this->model_sps_order->getOrderOptions($_SESSION['store_code'], $this->request->get['order_id'], $product['order_product_id']);
        	...
        	$this->data['products'][] = array(
             ...
             'discount' => $product['discount'] ? $this->currency->format($product['discount']) : NULL,
				 'total'    => $this->currency->format($product['total'], $order_info['currency'], $order_info['value']),
             'product_options_friendly' => $this->model_sps_order->get_order_options_formatted($_SESSION['store_code'], $this->request->get['order_id'], $product['product_id'])
    ...
    // Shipping Methods
      ...
      //var_dump($this->customer->getSPS()->hasFreeShipping($this->cart->getSubtotal()));
      //var_dump($this->cart->getSubtotal() > $this->config->get('free_total'));
      //var_dump($has_extra_shipping_item);
		foreach ($shipping_extensions as $shipping_extension) {
		    
         // SPS (check for free shipping).
         if ($this->customer->isSPS() && $shipping_extension['key'] == 'free') {
            if ( ($this->customer->getSPS()->hasFreeShipping($this->cart->getSubtotal()) ||
                  $this->cart->getSubtotal() > $this->config->get('free_total')) && (!$has_extra_shipping_item)) {
...
//exit;
    ...
    // end Shipping Methods
    	...
    	$this->data['totals'] = $this->model_sps_order->getOrderTotals($_SESSION['store_code'], $this->request->get['order_id']);

    	$this->data['historys'] = array();

    	$results = $this->model_sps_order->getOrderHistory($_SESSION['store_code'], $this->request->get['order_id']);
    	...
    	$results = $this->model_sps_order->getOrderDownloads($_SESSION['store_code'], $this->request->get['order_id']);
      ...
      // Gather the approvers that would be available for this order.
      $this->load->model('sps/chain');
      $this->data['order_approvers'] = $this->model_sps_chain->getApprovalChainUsers($order_info['school_id']);
      $this->data['user_id'] = $this->user->getSPS()->getUserID();

      // Gather approval audit trail
      $this->data['approval_audits'] = $this->model_sps_order->getOrderAudit($this->request->get['order_id']);

		$this->data['comment'] = @$this->request->post['comment'];
		$this->data['notify'] = @$this->request->post['notify'];
	
		$this->id       = 'content';
		$this->template = 'sps/order_form.tpl';
		$this->layout   = 'common/layout';
	...
	public function invoice() {
    	...
    	foreach ($products as $product) {
        	...
        	$this->data['products'][] = array(
          		...
          		'quantity' => $product['quantity'],
          	   'price'    => $this->currency->format($product['price']),
               'discount' => (int)$product['discount'] ? $this->currency->format($product['discount']) : NULL,
				   'total'    => $this->currency->format($product['total'], $order_info['currency'], $order_info['value']),
        		   'product_options_friendly' => $this->model_sps_order->get_order_options_formatted($_SESSION['store_code'], $this->request->get['order_id'], $product['product_id'])
        	);
    	}
		...
		$this->template = 'sps/order_invoice.tpl';
	...
	private function validate ($order_id, $viewing_user_id) {
	   ...
	   $this->load->model('sps/order');
    	...
    	if (!$this->user->hasPermission('modify', 'sps/order')) {
  	...
  	public function process_form_product_updates ($return_mode=null, $shipping_selection, $order_id) {
       ...
       $current_order_products = $this->model_sps_order->getOrderProducts($_SESSION['store_code'], $order_id);
	    ...
	    foreach ((array)$distinct_product_ids as $product_id => $product_data) {
	        $this->cart->add(
	            ...
	            array(
                  ...
                  'total' => (int)$detail_order_products[$product_id]['discount'] ? ($detail_order_products[$product_id]['discount']*$product_data['quantity']) : ($product_data['price']*$product_data['quantity']),
...
//	   print_r ($updated_cart_data);  exit;
  	...
  	public function init_order_cart () {
  	    ...
  	    $this->cart = new Cart();
  	    $this->cart->clear();  	    
  	    $this->session->data['customer_id'] = $this->model_sps_order->get_customer_id($_REQUEST['order_id']);
  	    $this->session->data['order_id_'.$_REQUEST['order_id']]['shipping_address_id'] = $this->model_sps_order->get_shipping_address_id($_REQUEST['order_id']);

  	    $this->load->library('customer');
  	    $this->cart->customer = new Customer();
//var_dump($this->cart->customer);
...
//exit;
      ...
      */
      $this->model_sps_order->update($order_id, $data);
  	}
