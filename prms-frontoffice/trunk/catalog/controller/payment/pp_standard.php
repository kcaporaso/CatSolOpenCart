...
class ControllerPaymentPPStandard extends Controller {
	...
	public function callback() {
		...
		$order_info = $this->model_checkout_order->getOrder($_SESSION['store_code'], $order_id);
			...
			if ($fp) {
				...
				while (!feof($fp)) {
					...
					if (strcmp($response, 'VERIFIED') == 0) {
						$this->model_checkout_order->confirm($_SESSION['store_code'], $order_id, $this->config->get('config_order_status_id'));
						
						switch($this->request->post['payment_status']){
							case 'Completed':
								$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('pp_standard_order_status_id'), '', TRUE);
								break;
							case 'Canceled_Reversal':
								$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('pp_standard_order_status_canceled_reversal_id'), '', TRUE);
								break;
							case 'Denied':
								$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('pp_standard_order_status_denied_id'), '', TRUE);
								break;
							case 'Failed':
								$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('pp_standard_order_status_failed_id'), '', TRUE);
								break;
							case 'Pending':
								$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('pp_standard_order_status_pending_id'), '', TRUE);
								break;
							case 'Refunded':
								$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('pp_standard_order_status_refunded_id'), '', TRUE);
								break;
							case 'Reversed':
								$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('pp_standard_order_status_reversed_id'), '', TRUE);
								break;
							default:
								$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('pp_standard_order_status_unspecified_id'), '', TRUE);
								break;
						}
					}
				}
