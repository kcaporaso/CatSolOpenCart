	...
	public function callback() {
		...
		if ($order_info) {
			...
			switch($this->request->post['status']) {
				case '2':
					$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('moneybookers_order_status_id'), '', TRUE);
					break;
				case '0':	
					$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('moneybookers_order_status_pending_id'), '', TRUE);		
					break;
				case '-1':
					$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('moneybookers_order_status_canceled_id'), '', TRUE);
					break;
				case '-2':
					$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('moneybookers_order_status_failed_id'), '', TRUE);
					break;					
				case '-3':
					$this->model_checkout_order->update($_SESSION['store_code'], $order_id, $this->config->get('moneybookers_order_status_chargeback_id'), '', TRUE);
					break;
			}
		}
	}
