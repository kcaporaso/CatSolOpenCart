...
class ControllerCheckoutAddress extends Controller {
	...
	public function shipping() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && (@$this->request->post['address_id'])) {
         ...
         //var_dump($this->request->post['address_id']);
         //exit;
  	...
  	public function payment() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && (@$this->request->post['address_id'] || @$this->request->post['billing_address_id'])) {
	  		unset($this->session->data['payment_methods']);
			unset($this->session->data['payment_method']);
			
         if (isset($this->request->post['billing_address_id'])) {
            $this->session->data['use_billing_address'] = true;
			   $this->session->data['payment_address_id'] = $this->request->post['billing_address_id'];
         } else {
            $this->session->data['use_billing_address'] = false;
      ...
      if ($type == 'payment') {
         // Here we show the billing address for the school or districts.
         $billing_addresses = $this->model_account_address->getBillingAddresses();
         foreach ($billing_addresses as $addr) {
            $results[] = $addr;
         }
      }

      $this->data['show_new_address_form'] = true;
      if ($this->customer->isSPS()) {
         $this->data['show_new_address_form'] = false;
      }   

		foreach ($results as $result) {
      	$this->data['addresses'][] = array(
        		'address_id' => $result['address_id'],
	    		'address'    => $result['firstname'] . ' ' . $result['lastname'] . ', ' . $result['company'] . ', ' . $result['address_1'] . ', ' . $result['city'] . ', ' . (($result['zone']) ? $result['zone']  . ', ' : FALSE) . $result['country'],
            'type' => $result['type'],
  	...
  	private function validate() {
    	...
    	if ((strlen(utf8_decode($this->request->post['address_1'])) < 3) || (strlen(utf8_decode($this->request->post['address_1'])) > 64)) {
  	    ...
  	    if ((strlen(utf8_decode($this->request->post['postcode'])) < 3) || (strlen(utf8_decode($this->request->post['postcode'])) > 32)) {
