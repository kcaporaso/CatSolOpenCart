...
class ControllerAccountInvoice extends Controller {
	...
	public function index() { 
      ...
      if ($this->customer->isSPS()) {
		   ...
		   $order_info = $this->model_account_order->getOrder($_SESSION['store_code'], @$this->request->get['order_id']);
      }
		...
		if ($order_info) {
         ...
         if ($this->customer->isSPS()) {
			   ...
			   $products = $this->model_account_order->getOrderProducts($_SESSION['store_code'], $this->request->get['order_id']);
         }
            ...
            if ($this->customer->isSPS()) {
				   ...
				   $options = $this->model_account_order->getOrderOptions($_SESSION['store_code'], $this->request->get['order_id'], $product['order_product_id']);
            }
            ...
            // KMC  
            /*
             * DOn't think this is needed, the discount price is pulled out of the order_products
            $this->data['category_discounts'] = $this->customer->getCategoryDiscounts();
            $category_id = '';
            $this->load->model('catalog/category');
//echo 'order_product_id: ' . $product['order_product_id'].'<br/>';
...
//echo 'cat_id: ' . $category_id . '<br/>';
            ...
            if ($this->data['category_discounts']>0) {
               foreach ($this->data['category_discounts'] as $discount) {
                  if (array_key_exists($category_id, $discount)) {
                     $this->data['have_category_discounts'] = true; 
                     $discount_pct = $discount[$category_id];
                     break;
                  }       
               }       
...
//echo 'discout: ' . $product['discount'] . '<br/>';
         ...
         if ($this->customer->isSPS()) {
      		...
      		$this->data['totals'] = $this->model_account_order->getOrderTotals($_SESSION['store_code'], $this->request->get['order_id']);
         }
         ...
         if ($this->customer->isSPS()) {
			   $results = $this->model_sps_order->getOrderHistorys($_SESSION['store_code'], $this->request->get['order_id']);
         } else {
			   $results = $this->model_account_order->getOrderHistorys($_SESSION['store_code'], $this->request->get['order_id']);
         }
            ...
            if ($this->customer->isSPS()) {
               if ($this->model_sps_order->isOrderRejected($_SESSION['store_code'], $this->request->get['order_id'])) {
      		      $this->data['fix_order_url'] = $this->url->https('sps/fix_order&order_id='.$this->request->get['order_id']);
                  $this->data['button_fix_order'] = $this->language->get('button_fix_order');
               }
            }
