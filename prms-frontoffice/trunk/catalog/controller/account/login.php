...
class ControllerAccountLogin extends Controller {
	...
	public function index() {
      ...
      // do some impersonate stuff for sps back-end.
      if (strpos($_SERVER['HTTP_REFERER'], 'http://admin.') !== false &&
          strpos($_SERVER['HTTP_REFERER'], 'sps/user') !== false) {
         session_id($this->request->get['k']);
         session_start();
         //var_dump($_SESSION);
         if ($_SESSION['imp_key'] == '2f053966e02661fd6bddc91317990e0c') {
            //echo 'success';
         }
      }
...
//                    var_dump($this->customer);exit;
  	...
  	private function validate() {
    	if (!$this->customer->login($_SESSION['store_code'], $this->request->post['email'], $this->request->post['password'])) {
