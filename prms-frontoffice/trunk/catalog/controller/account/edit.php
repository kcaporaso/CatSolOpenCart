...
class ControllerAccountEdit extends Controller {
	...
	private function validate() {
		...
		if ((strlen(utf8_decode($this->request->post['email'])) > 32) || (!preg_match('#^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$#i', $this->request->post['email']))) {
			$this->error['email'] = $this->language->get('error_email');
		}
		
		if (($this->customer->getEmail() != $this->request->post['email']) && ($this->model_account_customer->getTotalCustomersByEmail($_SESSION['store_code'], $this->request->post['email']))) {
