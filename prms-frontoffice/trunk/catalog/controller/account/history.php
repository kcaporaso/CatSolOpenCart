...
class ControllerAccountHistory extends Controller {	
	public function index() {
      ...
      if ($this->customer->isSPS()) {
		   ...
		   $order_total = $this->model_sps_order->getTotalOrders($_SESSION['store_code']);
      } else {
		   $this->load->model('account/order');
		   $order_total = $this->model_account_order->getTotalOrders($_SESSION['store_code']);
      }
		...
		if ($order_total) {
         ...
         if ($this->customer->isSPS()) {
			   $results = $this->model_sps_order->getOrders($_SESSION['store_code'], ($page - 1) * 20, 20);
         } else {
			   $results = $this->model_account_order->getOrders($_SESSION['store_code'], ($page - 1) * 20, 20);
         }
      		
			foreach ($results as $result) {
            if ($this->customer->isSPS()) {
        		   $product_total = $this->model_sps_order->getTotalOrderProductsByOrderId($_SESSION['store_code'], $result['order_id']);
            } else {
        		   $product_total = $this->model_account_order->getTotalOrderProductsByOrderId($_SESSION['store_code'], $result['order_id']);
            }
